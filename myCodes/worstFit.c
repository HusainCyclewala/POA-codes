...
void main()
{
    ...
    */
    ...
    // arranging memory blocks in descending order of their size.
    for (int i = 0; i < n_mem-1 ; i++)          //Only use memory here.
    {
    ...
    //assigning the blocks to processes.
    for (int i = 0; i < n_proc; i++)
    {
        if (process_sizes[i] <= mem_sizes[index])    // Use index here. 
        {
            mem_loc[i] = index;
            index++;
        }
